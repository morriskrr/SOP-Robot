## Overview
This package contains three nodes, **face_recognition**, **emotion_detection** and **action_client**. Face_recogniton node detects a largest face from webcam image and sends the cropped image of the face to emotion_detection node which in turn runs inference and finds out the emotion on the face. Emotion_detection node then sends that emotion as a string to action_client node that controls a robot. Based on what emotion it received it send different actions.
